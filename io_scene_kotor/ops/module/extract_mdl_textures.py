...
class KB_OT_extract_mdl_textures(bpy.types.Operator, ExportHelper):
    ...
    def _execute_extract_mdl_textures_core(self, context: bpy.types.Context) -> set[str]:
    ...
    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.module.extract_mdl_textures")
        session = begin_import_operator_diag(
            log, "kb.extract_mdl_textures", self, self.filepath or ""
        )
        outcome = "FINISHED"
        work_done = False
        ...
        try:
            ret = self._execute_extract_mdl_textures_core(context)
