...
class KB_OT_new_utm(bpy.types.Operator, ExportHelper):
    bl_idname = "kb.new_utm"
    bl_label = "New Merchant (UTM)"
    bl_description = "Create a new KotOR Merchant resource file"

    filename_ext = ".utm"
    filter_glob: bpy.props.StringProperty(default="*.utm", options={"HIDDEN"})
    ...
    def _execute_new_utm_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Created new UTM file: {self.filepath}")
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.resource.new_utm")
        session = begin_import_operator_diag(log, "kb.new_utm", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_new_utm_core(context)
