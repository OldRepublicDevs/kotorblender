...
class KB_OT_new_uti(bpy.types.Operator, ExportHelper):
    bl_idname = "kb.new_uti"
    bl_label = "New Item (UTI)"
    bl_description = "Create a new KotOR Item resource file"

    filename_ext = ".uti"
    filter_glob: bpy.props.StringProperty(default="*.uti", options={"HIDDEN"})
    ...
    def _execute_new_uti_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Created new UTI file: {self.filepath}")
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.resource.new_uti")
        session = begin_import_operator_diag(log, "kb.new_uti", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_new_uti_core(context)
