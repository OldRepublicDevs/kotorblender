...
class KB_OT_new_utd(bpy.types.Operator, ExportHelper):
    bl_idname = "kb.new_utd"
    bl_label = "New Door (UTD)"
    bl_description = "Create a new KotOR Door resource file"

    filename_ext = ".utd"
    filter_glob: bpy.props.StringProperty(default="*.utd", options={"HIDDEN"})
    ...
    def _execute_new_utd_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Created new UTD file: {self.filepath}")
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.resource.new_utd")
        session = begin_import_operator_diag(log, "kb.new_utd", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_new_utd_core(context)
