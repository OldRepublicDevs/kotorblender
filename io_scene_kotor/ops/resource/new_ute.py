...
class KB_OT_new_ute(bpy.types.Operator, ExportHelper):
    bl_idname = "kb.new_ute"
    bl_label = "New Trigger (UTE)"
    bl_description = "Create a new KotOR Trigger resource file"

    filename_ext = ".ute"
    filter_glob: bpy.props.StringProperty(default="*.ute", options={"HIDDEN"})
    ...
    def _execute_new_ute_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Created new UTE file: {self.filepath}")
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.resource.new_ute")
        session = begin_import_operator_diag(log, "kb.new_ute", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_new_ute_core(context)
