...
class KB_OT_new_tlk(bpy.types.Operator, ExportHelper):
    bl_idname = "kb.new_tlk"
    bl_label = "New Talk Table (TLK)"
    bl_description = "Create a new KotOR Talk Table resource file"

    filename_ext = ".tlk"
    filter_glob: bpy.props.StringProperty(default="*.tlk", options={"HIDDEN"})
    ...
    def _execute_new_tlk_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Created new TLK file: {self.filepath}")
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.resource.new_tlk")
        session = begin_import_operator_diag(log, "kb.new_tlk", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_new_tlk_core(context)
