...
class KB_OT_new_utw(bpy.types.Operator, ExportHelper):
    ...
    def _execute_new_utw_core(self, context: bpy.types.Context) -> set[str]:
    ...
    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.resource.new_utw")
        session = begin_import_operator_diag(log, "kb.new_utw", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_new_utw_core(context)
