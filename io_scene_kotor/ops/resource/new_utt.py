...
class KB_OT_new_utt(bpy.types.Operator, ExportHelper):
    bl_idname = "kb.new_utt"
    bl_label = "New Trap (UTT)"
    bl_description = "Create a new KotOR Trap resource file"

    filename_ext = ".utt"
    filter_glob: bpy.props.StringProperty(default="*.utt", options={"HIDDEN"})
    ...
    def _execute_new_utt_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Created new UTT file: {self.filepath}")
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.resource.new_utt")
        session = begin_import_operator_diag(log, "kb.new_utt", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_new_utt_core(context)
