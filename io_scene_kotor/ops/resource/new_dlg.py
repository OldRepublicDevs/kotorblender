...
class KB_OT_new_dlg(bpy.types.Operator, ExportHelper):
    bl_idname = "kb.new_dlg"
    bl_label = "New Dialog (DLG)"
    bl_description = "Create a new KotOR Dialog resource file"

    filename_ext = ".dlg"
    filter_glob: bpy.props.StringProperty(default="*.dlg", options={"HIDDEN"})
    ...
    def _execute_new_dlg_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"New DLG creation not yet implemented. Path: {self.filepath}")
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.resource.new_dlg")
        session = begin_import_operator_diag(log, "kb.new_dlg", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_new_dlg_core(context)
