...
class KB_OT_new_utp(bpy.types.Operator, ExportHelper):
    bl_idname = "kb.new_utp"
    bl_label = "New Placeable (UTP)"
    bl_description = "Create a new KotOR Placeable resource file"

    filename_ext = ".utp"
    filter_glob: bpy.props.StringProperty(default="*.utp", options={"HIDDEN"})
    ...
    def _execute_new_utp_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Created new UTP file: {self.filepath}")
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.resource.new_utp")
        session = begin_import_operator_diag(log, "kb.new_utp", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_new_utp_core(context)
