...
class KB_OT_extract_save(bpy.types.Operator, ImportHelper):
    bl_idname = "kb.extract_save"
    bl_label = "Extract Save Game"
    bl_description = "Extract resources from a KotOR save game file"
    ...
    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.save.extract")
        session = begin_import_operator_diag(
            log, "kb.extract_save", self, self.filepath or ""
        )
        outcome = "FINISHED"
        work_done = False
        ...
        try:
            if not is_pykotor_available():
                ...
                self.report(
                    {"INFO"},
                    f"Save game extraction not yet implemented. Path: {self.filepath}",
                )
                work_done = True
                reason_code = LogReasonCode.OK
                outcome = "FINISHED"
