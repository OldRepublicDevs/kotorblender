...
class KB_OT_edit_uts(bpy.types.Operator, ImportHelper):  # pyright: ignore[reportIncompatibleMethodOverride]
    bl_idname = "kb.edit_uts"
    bl_label = "Edit Sound"
    bl_description = "Edit a KotOR Sound (UTS) file"

    filename_ext = ".uts"
    filter_glob: bpy.props.StringProperty(default="*.uts", options={"HIDDEN"})  # pyright: ignore[reportInvalidTypeForm]
    ...
    def _execute_edit_uts_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Editing UTS file: {self.filepath}")  # pyright: ignore[reportAttributeAccessIssue]
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.editor.edit_uts")
        session = begin_import_operator_diag(log, "kb.edit_uts", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_edit_uts_core(context)
