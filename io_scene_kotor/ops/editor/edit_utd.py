...
class KB_OT_edit_utd(bpy.types.Operator, ImportHelper):  # pyright: ignore[reportIncompatibleMethodOverride]
    bl_idname = "kb.edit_utd"
    bl_label = "Edit Door"
    bl_description = "Edit a KotOR Door (UTD) file"

    filename_ext = ".utd"
    filter_glob: bpy.props.StringProperty(default="*.utd", options={"HIDDEN"})  # pyright: ignore[reportInvalidTypeForm]
    ...
    def _execute_edit_utd_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Editing UTD file: {self.filepath}")  # pyright: ignore[reportAttributeAccessIssue]
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.editor.edit_utd")
        session = begin_import_operator_diag(log, "kb.edit_utd", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_edit_utd_core(context)
