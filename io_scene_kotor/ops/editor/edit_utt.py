...
class KB_OT_edit_utt(bpy.types.Operator, ImportHelper):  # pyright: ignore[reportIncompatibleMethodOverride]
    bl_idname = "kb.edit_utt"
    bl_label = "Edit Trigger"
    bl_description = "Edit a KotOR Trigger (UTT) file"

    filename_ext = ".utt"
    filter_glob: bpy.props.StringProperty(default="*.utt", options={"HIDDEN"})  # pyright: ignore[reportInvalidTypeForm]
    ...
    def _execute_edit_utt_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Editing UTT file: {self.filepath}")  # pyright: ignore[reportAttributeAccessIssue]
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.editor.edit_utt")
        session = begin_import_operator_diag(log, "kb.edit_utt", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_edit_utt_core(context)
