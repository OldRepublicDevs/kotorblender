...
class KB_OT_edit_erf(bpy.types.Operator, ImportHelper):  # pyright: ignore[reportIncompatibleMethodOverride]
    ...
    def _execute_edit_erf_core(self, context: bpy.types.Context) -> set[str]:
    ...
    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.editor.edit_erf")
        session = begin_import_operator_diag(log, "kb.edit_erf", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_edit_erf_core(context)
