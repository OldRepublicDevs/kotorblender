...
class KB_OT_edit_uti(bpy.types.Operator, ImportHelper):  # pyright: ignore[reportIncompatibleMethodOverride]
    bl_idname = "kb.edit_uti"
    bl_label = "Edit Item"
    bl_description = "Edit a KotOR Item (UTI) file"

    filename_ext = ".uti"
    filter_glob: bpy.props.StringProperty(default="*.uti", options={"HIDDEN"})  # pyright: ignore[reportInvalidTypeForm]
    ...
    def _execute_edit_uti_core(self, context: bpy.types.Context) -> set[str]:
        if not is_pykotor_available():
            self.report({"ERROR"}, "PyKotor is not available. Install PyKotor to use this feature.")
            return {"CANCELLED"}

        self.report({"INFO"}, f"Editing UTI file: {self.filepath}")  # pyright: ignore[reportAttributeAccessIssue]
        return {"FINISHED"}

    def execute(self, context: bpy.types.Context) -> set[str]:
        log = get_kb_logger("ops.editor.edit_uti")
        session = begin_import_operator_diag(log, "kb.edit_uti", self, self.filepath or "")
        outcome = "FINISHED"
        work_done = False
        reason_code = LogReasonCode.OK
        ret: set[str] = {"CANCELLED"}
        try:
            ret = self._execute_edit_uti_core(context)
